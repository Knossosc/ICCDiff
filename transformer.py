...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # padding
        _, _, H, W = x.size()
        if W % self.patch_size[1] != 0: # 对输入的张量 x 进行填充操作，以确保其尺寸可以被 patch 的大小整除
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=1, in_chans=320 * 6, num_classes=1000,
                 embed_dim=32, depths=[2, 2, 6, 2], num_heads=[2, 4, 8, 16],
                 window_size=(7, 7), mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, num_patches=24 * 24, **kwargs):
        super().__init__()
        ...
        patches_resolution = [24, 24]
        self.patches_resolution = [24, 24]
    ...
    def forward(self, x):
        x = self.patch_embed(x)  # batch * (96) * 56*56
        x = self.pos_drop(x)
        # print(x.shape)
        feat_list = []
        for layer in self.layers:  # 4层
            x = layer(x.contiguous())  # 输入张量的内存连续性
            # print(x.shape)
            feat_list.append(x)  # 4个特征层
...
if __name__ == '__main__':
    model = SwinTransformer(img_size=224, patch_size=1, in_chans=3, num_classes=1, drop_path_rate=0.2).cuda()
    sr = torch.zeros(1, 3, 400, 608).cuda()
